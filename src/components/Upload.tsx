import ulploadSvg from "../assets/upload.svg";
  ...
  return (
    <div className="mb-4">
      ...
      <div className="w-full h-12 flex items-center rounded-lg border border-gray-300 text-sm text-gray-100 bg-transparent outline-none">
        ...
        <span className="text-xs text-gray-1oo flex-1 pl-4">
          {filename ?? "Selecione o arquivo"}
        </span>

        <label
          htmlFor="upload"
          className="flex h-12 px-4 items-center bg-green-100 rounded-r-lg cursor-pointer disabled:opacity-50 hover:bg-green-200 transition ease-linear"
        >
          <img src={ulploadSvg} alt="icone de upload" className="w-6 h-6" />
        </label>
      </div>
    </div>
  );
